...
output$plot_all_or_DE <- renderUI({
  if(!input$genelist %in% c(" ", "Built-in gene list")) {
      radioButtons("all_or_DE", 
                ...
                choices = c("All genes in list", "Only DE genes (5%)", "Only DE genes (1%)"),
...
heatmap_data_select <- reactive({
    ...
    if(input$genelist != "Built-in gene list") {
      if(input$all_or_DE == "Only DE genes (1%)") {
#      DE_index <- which(overall_LRT_pvalues$qval < 0.05)
      ...
      } else if(input$all_or_DE == "Only DE genes (5%)") {
       available_transcripts <- rownames(LFC_all)
...
nonkmeans_labs <- reactive({
  ...
  if(input$cluster_rows != "K-means") {
      ...
      pvaldf <- full_join(overall_LRT_pvalues, 
                          data.frame(target_id = rownames(qval_all), qval_all, check.names = FALSE),
                          by = "target_id") %>% 
        dplyr::select(-qval)
      colnames(pvaldf) <- c("ID", "ext_gene", "2dpi-0dpi_qval", "4dpi-0dpi_qval",
                            "7dpi-0dpi_qval", "12dpi-0dpi_qval")
      LFCdf <- data.frame(ID = rownames(LFC_all), LFC_all, check.names=FALSE)
...
Transcript abundances in the RNA-seq data were quantified using [kallisto](https://pachterlab.github.io/kallisto/), and differentially expressed transcripts were identified using [sleuth](https://pachterlab.github.io/sleuth/). For heatmaps of built-in differential gene lists (e.g. "2dpi-0dpi DE genes"), a q-value threshold of 1% was used to identify DE transcripts; the "All DE genes" list corresponds to the 4614 transcripts for which a q-value < 1% was observed for at least one of the pairwise comparisons. When reading in an Excel spreadsheet or .csv file containing gene symbols or Ensembl Ids, users additionally have the option of filtering their gene selection to include only DE genes; in this case, genes are identified as DE if the q-value from at least one pairwise comparison is less than 1% or 5% for any transcript corresponding to the gene.
