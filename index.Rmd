...
output$plot_all_or_DE <- renderUI({
  if(!input$genelist %in% c(" ")) {
    if(input$genelist == "Built-in gene list") {
          req(input$builtin_choice)
          if(!input$builtin_choice %in% c("all DE genes", "2dpi-0dpi DE genes", 
                                     "4dpi-0dpi DE genes", 
                        "7dpi-0dpi DE genes",  "12dpi-0dpi DE genes",
                      "growth toward the midline", "midline crossing",
                      "target selection", "brain innervation", 
                      "downregulated during regeneration - early",
                      "downregulated during regeneration - mid",
                      "downregulated during regeneration - late" )) {
...
output$peak_build_message <- renderUI({
  if(input$genelist != " ") {
    htmlOutput("peak_build_text")
  }
})
...
output$peak_build_text <- renderUI({
  HTML(paste("Note: peak coordinates correspond to the Danio rerio GRCz10 genome build. Only genomic regions overlapping ATAC-seq peaklets are used in this analysis. The UCSC Genome Browser", a("liftOver", target="_blank", href="https://genome.ucsc.edu/cgi-bin/hgLiftOver"), "tool can be used to convert genome coordinates to the Danio rerio GRCz11 build, if desired.", br(), br()))
})
```
...
heatmap_data_select <- reactive({
  ...
  if(input$genelist != " ") {
    ...
    }   else if(input$genelist == "Read in Excel sheet or .csv") {
#      index <- intersect(gene_select_index(), which(rowSums(is.na(LFC_all)) < 4))
      req(input$excel)
      req(input$all_or_DE)
        if(input$all_or_DE == "Only DE (padj<1%, Wald tests vs 0dpi)") {
...
h <- reactive({
  ...
  if(input$cluster_rows == "K-means") {
    req(input$kmeans_K)
    req(input$kmeans_order)
    req(input$cluster_cols)
    ## ADDED AUGUST 2019 !!!!!!!!
...
Note: peak coordinates correspond to the Danio rerio GRCz10 genome build. Only genomic regions overlapping ATAC-seq peaklets are used in this analysis. The UCSC Genome Browser [liftOver](https://genome.ucsc.edu/cgi-bin/hgLiftOver) tool can be used to convert genome coordinates to the Danio rerio GRCz11 build, if desired.
...
**6. What built-in gene lists are available in the app? **

For convenience, in addition to allowing users to input arbitrary regeneration-related gene lists in the app, we have included a variety of built-in gene lists. These include the following gene lists from a variety of different cell/tissue types and organisms (with corresponding references):

* Differential transcripts at each time point identified in Dhara et al. (2019): **2dpi-0dpi DE genes**, **4dpi-0dpi DE genes**, **7dpi-0dpi DE genes**, **12dpi-0dpi DE genes**

* Stage-specific clusters of transcripts visualized in Figure 1 of Dhara et al. (2019): **growth toward the midline**, **midline crossing**, **target selection**, **brain innervation**, **downregulated during regeneration - early**, **downregulated during regeneration - mid**, **downregulated during regeneration - late**

Dhara et al. (2019) Regulation of CNS regeneration-associated genes is driven by a temporally changing cast of transcription factors. *bioRxiv*, https://doi.org/10.1101/638734.




* **(Belin 2015) Mouse ONI Intact v Axotomized RGC**

Belin et al. (2015) Injury-induced decline of intrinsic regenerative ability revealed by quantitative proteomics. *Neuron.* 86(4):1000-1014. https://doi.org/10.1016/j.neuron.2015.03.060. PMID: 25937169; PMCID: PMC4551425.

* **(Chandran 2016) Mouse PNS regeneration**

Chandran et al. (2016) A Systems-Level Analysis of the Peripheral Nerve Intrinsic Axonal Growth Program. *Neuron.* 89(5):956-70. https://doi.org/10.1016/j.neuron.2016.01.034. PMID: 26898779; PMCID: PMC4790095. 

* **(Crippa 2016) Mouse Heart regeneration**
* **(Crippa 2016) Zebrafish Heart regenerationn**

Crippa et al. (2016) Comparative transcriptome profiling of the injured zebrafish and mouse hearts identifies miRNA-dependent repair pathways. *Cardiovasc Res.* 110(1):73-84. https://doi.org/10.1093/cvr/cvw031. PMID: 26857418; PMCID: PMC4798047.

* **(Dwaraka 2018) Salamander Limb regeneration**

Dwaraka et al. (2018) Comparative transcriptomics of limb regeneration: Identification of conserved expression changes among three species of *Ambystoma*. *Genomics*,  https://doi.org/10.1016/j.ygeno.2018.07.017.

* **(Herman 2018) Lamprey SCI Brain response**
* **(Herman 2018) Lamprey SCI SC response**

Herman et al. (2018) Highly conserved molecular pathways, including Wnt signaling, promote functional recovery from spinal cord injury in lampreys. *Sci Rep.* 8(1):742. https://doi.org/10.1038/s41598-017-18757-1. PMID: 29335507; PMCID: PMC5768751.

* **(Li 2013) Rat SNI proximal segment response**

Li et al. (2013) Differential gene expression profiling and biological process analysis in proximal nerve segments after sciatic nerve transection. *PLoS One*. 8(2):e57000. https://doi.org/10.1371/journal.pone.0057000. PMID: 23437294; PMCID: PMC3578805.

* **MGI cholesterol metabolic**

List of 125 genes with cholesterol-related GO terms obtained from the [Mouse Genome Informatics](http://www.informatics.jax.org/) (MGI) database.

* **(Rabinowitz 2017) Zebrafish Fin regeneration**
Rabinowitz et al. (2017) Transcriptomic, proteomic, and metabolomic landscape of positional memory in the caudal fin of zebrafish. *Proc Natl Acad Sci.* 114(5):E717-E726. https://doi.org/10.1073/pnas.1620755114. PMID: 28096348; PMCID: PMC5293114.

* **(Sauls 2018) Mouse embryonic fibroblast cardiac reprogramming**
Sauls et al. (2018) Initiating Events in Direct Cardiomyocyte Reprogramming. *Cell Rep.* 22(7):1913-1922. https://doi.org/10.1016/j.celrep.2018.01.047. PMID: 29444441; PMCID: PMC6045814.

* **(Sifuentes 2016) Zebrafish acute photic lesion Muller glia response**

Sifuentes et al. (2016) Rapid, Dynamic Activation of Müller Glial Stem Cell Responses in Zebrafish. *Invest Ophthalmol Vis Sci.* 57(13):5148-5160. https://doi.org/10.1167/iovs.16-19973. PMID: 27699411; PMCID: PMC5054728. 

* **(Umansky 2015) Mouse Muscle regeneration**

Umansky et al. (2015) Runx1 Transcription Factor Is Required for Myoblasts Proliferation during Muscle Regeneration. *PLoS Genet.* 11(8):e1005457. https://doi.org/10.1371/journal.pgen.1005457. PMID: 26275053; PMCID: PMC4537234.
