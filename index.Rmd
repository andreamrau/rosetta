...
[Sumona P. Dhara](https://uwm.edu/biology/people/dhara-sumona/), [Andrea Rau](https://andreamrau.github.io), Nicole M. Recka, [Paul L. Auer](http://people.uwm.edu/pauer/), [Michael J. Flister](https://www.mcw.edu/departments/physiology/faculty/michael-flister-phd), and [Ava J. Udvadia](https://uwm.edu/biology/people/udvadia-ava/)
...
output$excel_columns <- renderUI({
    if(input$genelist == "Read in Excel sheet or .csv") {
      ...
      return(NULL)
      inFile <- input$excel
      if(length(grep(".xlsx", inFile$datapath))) {
...
output$summary_text <- renderText({
  ...
  if(length(grep(".xlsx", input$excel$datapath))) {
...
output$plot_all_or_DE <- renderUI({
  if(!input$genelist %in% c(" ", "Built-in gene list")) {
      radioButtons("all_or_DE", 
                ...
                choices = c("All genes in list", "Only DE genes"),
                selected = "All genes in list", inline=TRUE) 
  }
})
## Clustering columns
...
gene_select_index <- reactive({
  ...
  # } else if(input$genelist == "Input my own list (Ensembl IDs)") {
  #   ## If zebrafish is chosen
  #   if(input$org_choice == "Danio rerio genes (GRCz10)") {
  #     if(length(grep("ENSDART", input$my_ensembl))) {
  #       search <- tokenize_words(as.character(input$my_ensembl),
  #                            lowercase=FALSE, strip_numeric=FALSE) %>% unlist()
  #       index <- which(available_transcripts %in% search)
  #     } else if(length(grep("ENSDARG", input$my_ensembl))) {
  #       search <- tokenize_words(as.character(input$my_ensembl),
  #                            lowercase=FALSE, strip_numeric=FALSE) %>% unlist()
  #       search <- data.frame(ens_gene = search) %>%
  #         left_join(., transcript_to_gene, by = "ens_gene") %>% dplyr::select(target_id) %>% unlist()
  #       index <- which(available_transcripts %in% search)
  #     } else {
  #       stop("Only gene or transcript Ensembl IDs may be used for zebrafish.")
  #     }
  #   ## Allow for any ensembl gene ids
  #   } else {
  #     library(biomaRt)
  #     search <- tokenize_words(as.character(input$my_ensembl), 
  #                           lowercase=FALSE, strip_numeric=FALSE) %>% unlist()
  ...
  #   }
    ...
    search <- tokenize_words(as.character(input$my_symbols),
                             lowercase=FALSE, strip_numeric=FALSE) %>% unlist()
    if(length(search)) {
      tmp <- left_join(data.frame(target_id = available_transcripts),
      ...
      if(input$hugo_vs_symbols == "HGNC gene symbols") {
        ...
        index <- which(tmp$ext_gene %in% search)
      }
    } else index <- c()
    ...
    if(length(search) > 0) {
      if(input$ensembl_vss_symbols == "Ensembl gene or transcript IDs") {
        ...
        ## Zebrafish Ensembl gene IDs
         ...
         ## Arbitrary Ensembl gene IDs (but they must ALL be Ensembl IDs)
        } else if (length(grep("ENS", search)) == length(search)) {
          library(biomaRt)
          ensembl <- useMart(host = 'mar2016.archive.ensembl.org', 
                   biomart='ENSEMBL_MART_ENSEMBL')
          neworg <- useDataset(ds[which(ds$description == input$org_choice),1], mart=ensembl)
...
heatmap_data_select <- reactive({
    ...
    if(input$genelist != "Built-in gene list") {
      if(input$all_or_DE == "Only DE genes") {
...
output$sequences <- renderUI({
  if(input$genelist == " " | nrow(h()@matrix) == 0) {
...
Transcript abundances in the RNA-seq data were quantified using [kallisto](https://pachterlab.github.io/kallisto/), and differentially expressed transcripts were identified using [sleuth](https://pachterlab.github.io/sleuth/). For heatmaps of built-in differential gene lists (e.g. "2dpi-0dpi DE genes"), a q-value threshold of 1% was used to identify DE transcripts; this corresponds to 4614 transcripts in the full DE list.  When reading in an Excel spreadsheet or .csv file containing gene symbols or Ensembl Ids, users additionally have the option of filtering their gene selection to include only DE genes; in this case, genes are identified as DE if the q-value from the overall likelihood ratio test of differential expression at any time point is less than 5% for any transcript corresponding to the gene.
...
No login or registration is required. This resource is available under the [GNU public license](https://www.gnu.org/licenses/gpl-3.0.en.html) (GPL-3). If you use any of the results or figures generated by this app in a publication or presentation, please cite our work: Dhara et al. (2019)  Identifying regulatory factors governing regeneration-associated gene expression during Central Nervous System (CNS) regeneration in zebrafish (in preparation) and Rau et al. (2019) Regeneration Rosetta:  An interactive web application to explore regeneration-associated gene expression and chromatin accessibility (in preparation).
...
* Dhara et al. (2019)  Identifying regulatory factors governing regeneration-associated gene expression during Central Nervous System (CNS) regeneration in zebrafish (in preparation)

For more information about this app:

* Rau et al. (2019) Regeneration Rosetta:  An interactive web application to explore regeneration-associated gene expression and chromatin accessibility (in preparation).
